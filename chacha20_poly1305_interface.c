...
void paddingCheck(poly1305_msg_struct  *in, uint8_t padding_length){
	uint8_t zeros_req1 = (strlen(in->aad)/2)%padding_length;
	zeros_req1 = 0 ? 0 : 16-zeros_req1;

	uint8_t zeros_req2 = (strlen(in->msg)/2)%padding_length;
	zeros_req2 = 0 ? 0 : 16-zeros_req2;

//	printf("pad1 len %d %s %d pad[ %s %d ]\n", zeros_req1, in->aad, strlen(in->aad), in->pad1, strlen(in->pad1));
...
//	printf("pad2 len %d %s %d pad[ %s %d ]\n", zeros_req2, in->msg, strlen(in->msg), in->pad2, strlen(in->pad2));
...
//rfc8439 section 2.8 https://tools.ietf.org/html/rfc8439#section-2.8
void aead_chacha20_poly1305(uint8_t *output,uint8_t *key, uint32_t key_len, uint8_t *nonce, uint8_t *plaintext_in, uint32_t len, const char *aad){
	...
	encrypt_init(ciphertext_out,poly_key, key_len, nonce, plaintext_in, len,counter); //encrypting plaintext
...
void aead_test(){
	...
	aead_chacha20_poly1305(mac_tag, key, 32, nonce, plaintext_in, plaintext_length, gaad);
...
void test_ec(){
	char k[]="1c9240a5eb55d38af333888604f6b5f0473917c1402b80099dca5cbc207075c0";
	char n[]="1000002000000002";
	char msg[]="2754776173206272696c6c69672c20616e642074686520736c6974687920746f7665730a446964206779726520616e642067696d626c6520696e2074686520776162653a0a416c6c206d696d737920776572652074686520626f726f676f7665732c0a416e6420746865206d6f6d65207261746873206f757467726162652e";
	u64 ctr=0x124542;
